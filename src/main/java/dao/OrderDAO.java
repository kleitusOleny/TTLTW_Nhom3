...
public class OrderDAO extends ADAO implements IDAO<Order, Integer> {
    ...
    public List<OrderViewModel> getAllOrdersWithStatus() {
        return jdbi.withHandle(handle -> handle.createQuery("""
                        SELECT o.id, o.create_at, o.total_price, o.is_delete,
    ...
    public java.util.Map<String, Object> getOrderInfo(int orderId) {
        return jdbi.withHandle(handle -> Objects.requireNonNull(handle.createQuery("""
                        SELECT o.id, o.user_id, o.shipping_address_id, o.discount_id, 
                               ...
                               pm.pay_strategy, pm.status as payment_status,
    ...
    public int countOrderIdLastWeek() {
        return jdbi.withHandle(handle -> handle.createQuery("SELECT COUNT(id) FROM orders WHERE create_at >= DATE_SUB(NOW(), INTERVAL 7 DAY) AND is_delete IS NULL")
                .mapTo(Integer.class)
                .findOnly());
    }

    public double sumTotalPriceLastMonth() {
        return jdbi.withHandle(handle -> handle.createQuery("SELECT SUM(total_price) FROM orders WHERE create_at >= DATE_SUB(NOW(), INTERVAL 1 MONTH) AND is_delete IS NULL")
                .mapTo(Double.class)
                .findOne().orElse(0.0));
    }
    ...
    public Order save(Order entity) {
        return jdbi.withHandle(handle -> {
            ...
            var update = handle.createUpdate(sql)
                    ...
                    .bind("update_at", entity.getUpdateAt())
                    .bind("note", entity.getNote())
                    .bind("message", entity.getMessage());
            if (isUpdate && exists) {
